...
class DetectionProcessor(Node):
    def __init__(self, model_path):
        super().__init__('detection_coco', namespace=ros_namespace())
        self.node_id = f"{resolve_device_hostname()}-detection_coco"
        ...
        self.det_pub = self.create_publisher(String, TOPIC_DETECTION_COCO, qos)
        self.sub = self.create_subscription(
            CompressedImage, TOPIC_MOTION_IMAGE, self.on_image, qos
        )

        logging.info(f"[DET_PUB:{self.node_id}] Publishing on topic '{TOPIC_DETECTION_COCO}'")
